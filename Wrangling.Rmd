...
1 = Not at all happy
```
Since the different between `3, 4` and `1, 2` are rather vaue (and per recommendation of a TF), we decided to treat this variable as binary for now with `3, 4` being `1` and `1, 2` being a `0`.
...
Out of this model, 4 very significant predictors of happiness arise: `country`, `citizen`, `happy`, and `town_size`. The significance of these variables is quite intuitive, however, some more significant variables are likely to be uncovered once we fix assumptions of violations (out response is binary!). 
